...
TEST(QuaternionTest, Inverse)
{
    ...
    EXPECT_NEAR(prod.x, 0, EPS);
    EXPECT_NEAR(prod.y, 0, EPS);
    EXPECT_NEAR(prod.z, 0, EPS);
    EXPECT_NEAR(prod.w, 1, EPS);
}
...
TEST(QuaternionTest, MultiplyQuaternion)
{
    ...
    EXPECT_NEAR(result.length(), 1.0f, EPS);
}
...
{
    Vector3 v(1, 0, 0);
    Vector3 r = Quaternion::from_euler(0, PI / 2, 0) * v;
...
TEST(QuaternionTest, AxisAngleConversion)
{
    ...
    EXPECT_NEAR(outAxis.x, axis.x, EPS);
    EXPECT_NEAR(outAxis.y, axis.y, EPS);
    EXPECT_NEAR(outAxis.z, axis.z, EPS);
    EXPECT_NEAR(outAngle, angle, EPS);
}
